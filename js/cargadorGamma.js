// Cargador gamma Configuracion


var urlBase = "http://localhost:8003/"
var urlSaveData = urlBase + 'controladores/savedata'
var roms = ["ROMS/pk.zip"];
...
// worker para comprimir partidas
var compresor = new Worker("./js/worker/compresorGammaV1.js");
// Decompresor sincrono (lo siento KISS y patrones :( )
...
// 	var self = this;
// 		var result = self.setRom(romFile);
// 		callback(result);
// };
...
GameBoyAdvance.prototype.storeSavedata = function() {
	...
	try {
		...
		// Esto guarda la partida gamma (se agrega un worker)
		var storage = window.localStorage;
		// Clave y valor del localStorage
		var clave = this.SYS_ID + '.' + this.mmu.cart.code;
		var valor = this.encodeBase64(sram.view);
		var datos = '';
// Iniciador del compresor

		var datosCompresion = {
			tipo: "comprime",
			datos: valor
		}
		compresor.postMessage(datosCompresion);
		compresor.addEventListener('message', function(e) {
			// Esto es importante ya que esto guarda en el localStorage
			datos = e.data;
			storage[clave] = datos;
			// Esto es el savedata upload en la base de datos
			var dat = {
...
GameBoyAdvance.prototype.retrieveSavedata = function() {
	...
	try {
		...
		var storage = window.localStorage;
		var valor = storage[this.SYS_ID + '.' + this.mmu.cart.code];
		// La decompresion tiene que ser sincrona ya que el software no admite decompresion asincrona :(
		this.decodeSavedata(compresorGamma.decomprimir(valor))
	} catch (e) {
...
// Iniciador del emulador
function run(file) {

 // Esto fuera, asi no ehh
...
// loadRom(roms[0], function(quesoi) {
// 	console.log(quesoi);
// 	run(quesoi);
// });
...
function gammaCharge(url) {
	var xhr = new XMLHttpRequest();
	xhr.open('GET', 'ROMS/pk.zip');
	xhr.responseType = 'blob';

	xhr.onprogress = function (e) {
		if (e.lengthComputable) {
			console.log(progressBar);
			progressBar.value = parseInt((e.loaded / e.total) * 90);
		}
	};
...
// function cargarSaveData(x) {
// 	var pokejax = new XMLHttpRequest();
// 	pokejax.open("post", 'http://localhost/controladores/getsavedata');
// 	pokejax.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
// 	pokejax.send(JSON.stringify(x));

// 	pokejax.onload = function() {
// 		if(this.status === 200) {
// 			var salida = JSON.parse(this.response);
// 			console.log(salida.save_name);
// 			localStorage.setItem(salida.save_name, salida.save_data);
// 		}
// 	}

// }

/* ################# Esto envia al servidor para guardar datos #################### */
function envioSaveData(x) {
	var pokejax = new XMLHttpRequest();
	pokejax.open("POST", urlBase + 'controladores/savedata');
	pokejax.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
	pokejax.send(JSON.stringify(x));
